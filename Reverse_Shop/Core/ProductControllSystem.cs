...
namespace Core
{
   public class ProductControllSystem
   {
       ...
       private static void ControllFunction()
       {
           while (true)
           {
               ...
               {
                   Time time = ProductWorker.TimeValueProduct(item.Id);
                   if (time.Hours < 0 || time.Minutes < 0)
                   {
                       Core.MailSender.SendMail(UserRepository.SearchUser(item.IdBuyer ?? 1).Email, "You Won!",
                           MailBody(item, true), null);
                       Core.MailSender.SendMail(UserRepository.SearchUser(item.IdSaler).Email, "Your goods was bought!",
